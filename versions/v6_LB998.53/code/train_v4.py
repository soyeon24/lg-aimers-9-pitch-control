...
# and mixing 25% of it lifts F to 638 -- +5.5 overall.
...
def blend(groups, X, f_spec=None):
    ...
    if f_spec is not None:
        ...
        if m.any():
            pf = np.mean([mo.predict_proba(X[m])[:, 1] for mo in f_spec["models"]],
                         axis=0)
...
def main():
    ...
    f_models = []
    for sd in F_SEEDS[:args.seeds] if args.seeds else F_SEEDS:
        t = time.time()
        m = HistGradientBoostingClassifier(categorical_features=cat_idx,
                                           random_state=sd, **dict(BASE, **F_PARAMS))
        m.fit(Xtr[mF], ytr[mF])
        f_models.append(m)
        print(f"  f_spec seed {sd} ({time.time() - t:.0f}s, {mF.sum()} rows)",
              flush=True)
    f_spec = dict(models=f_models, weight=F_WEIGHT, params=dict(BASE, **F_PARAMS))
    ...
    artifact = dict(groups=groups, f_spec=f_spec, columns=columns, cat_idx=cat_idx, ctx=ctx,
                    ...
                    f_weight=F_WEIGHT, fe_version="v6")
