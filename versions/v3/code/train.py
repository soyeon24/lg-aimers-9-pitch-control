...
용량이 다른 두 HistGBM을 섞는다. 강한 정규화 쪽(A)이 주력이고,
얕은 트리가 놓치는 상호작용을 B가 조금 보탠다. 실험 10에서 w=0.3이 최선.
...
# A: 주력. 용량을 키우면 오히려 나빠지는 구간이라 강하게 정규화한다.
PARAMS_A = dict(max_iter=400, learning_rate=0.05, max_leaf_nodes=15,
                l2_regularization=10.0, min_samples_leaf=10000, early_stopping=False)
SEEDS_A = [0, 1, 2, 3, 4, 5, 6, 7]
# B: 다양성용. 단독으로는 A보다 나쁘지만 섞으면 조금 낫다.
PARAMS_B = dict(max_iter=400, learning_rate=0.05, max_leaf_nodes=31,
                l2_regularization=1.0, min_samples_leaf=2000, early_stopping=False)
SEEDS_B = [0, 1, 2, 3]
BLEND_W = 0.3   # B의 비중
EPS = 1e-6
...
def fit_ensemble(X, y, cat_idx, params, seeds, tag):
    models = []
    for seed in seeds:
        t = time.time()
        ...
        m.fit(X, y)
        models.append(m)
        print(f"  {tag} seed {seed} ({time.time() - t:.0f}s)", flush=True)
    return models


def predict_blend(models_a, models_b, X, w=BLEND_W):
    pa = np.mean([m.predict_proba(X)[:, 1] for m in models_a], axis=0)
    pb = np.mean([m.predict_proba(X)[:, 1] for m in models_b], axis=0)
    return (1 - w) * pa + w * pb
...
def main():
    ...
    print(f"학습 시작 X={Xtr.shape}  (A {len(SEEDS_A)}개 + B {len(SEEDS_B)}개, w={BLEND_W})")
    models_a = fit_ensemble(Xtr, ytr, cat_idx, PARAMS_A, SEEDS_A, "A")
    models_b = fit_ensemble(Xtr, ytr, cat_idx, PARAMS_B, SEEDS_B, "B")

    # 보정 상수: 마지막 시즌 행에 대한 블렌드 평균 예측을 기준점으로 삼고,
    # 외삽한 다음 시즌 성공률까지의 차이를 logit 오프셋으로 굳힌다.
    last_mask = (tr.season == last_season).values
    ref_mean = float(predict_blend(models_a, models_b, Xtr[last_mask]).mean())
    logit_offset = float(logit(target_rate) - logit(ref_mean))
    print(f"기준({last_season}) 예측 평균 = {ref_mean:.4f} → logit_offset = {logit_offset:+.4f}")

    artifact = dict(models_a=models_a, models_b=models_b, blend_w=BLEND_W,
                    columns=columns, cat_idx=cat_idx,
                    logit_offset=logit_offset, target_rate=target_rate, priors=priors,
                    ref_mean=ref_mean, trained_seasons=seasons,
    ...
    if args.validate:
        ...
        raw = predict_blend(models_a, models_b, Xva)
        adj = sigmoid(logit(raw) + logit_offset)
        oracle = sigmoid(logit(raw) + (logit(yva.mean()) - logit(raw.mean())))
        only_a = np.mean([m.predict_proba(Xva)[:, 1] for m in models_a], axis=0)
        ref_a = np.mean([m.predict_proba(Xtr[last_mask])[:, 1] for m in models_a], axis=0).mean()
        adj_a = sigmoid(logit(only_a) - logit(ref_a) + logit(target_rate))
        print("\n=== 2024 홀드아웃 ===")
        print(f"실제 성공률       = {yva.mean():.4f} (외삽값 {target_rate:.4f}, "
              f"오차 {target_rate - yva.mean():+.4f})")
        print(f"A만 {len(SEEDS_A)}시드     score={comp_score(adj_a, yva):8.1f}")
        print(f"보정 없음         score={comp_score(raw, yva):8.1f}  mean={raw.mean():.4f}")
        print(f"블렌드+오프셋     score={comp_score(adj, yva):8.1f}  mean={adj.mean():.4f}")
