...
def main():
    ...
    models_a, models_b = art["models_a"], art["models_b"]
    w, columns = art["blend_w"], art["columns"]
    offset = art["logit_offset"]
    print(f"모델 로드: A {len(models_a)}개 + B {len(models_b)}개 (w={w}), "
          f"피처 {len(columns)}개, logit_offset={offset:+.4f} ({time.time() - t0:.1f}s)")
