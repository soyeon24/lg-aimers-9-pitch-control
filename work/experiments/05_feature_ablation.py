"""피처 엔지니어링 ablation — 2024 홀드아웃, 고정 오프셋 보정 절차 포함.

모든 파생 피처는 '그 행 하나만 보고' 계산된다 (행 독립 추론 규칙).
"""
import time
import numpy as np
import pandas as pd
...
# 리그 사전확률 — 학습 데이터에서만 계산
PRIOR = {
    "success": float(tr.control_success.mean()),
    "middle": float(tr.asof_pitcher_middle_rate.mean()),
    "ball": float(tr.asof_pitcher_ball_rate.mean()),
    "strike": float(tr.asof_pitcher_strike_rate.mean()),
    "reverse": float(tr.asof_pitcher_reverse_rate.mean()),
}
print("prior:", {k: round(v, 4) for k, v in PRIOR.items()})
...
def add_count(X):
    ...
    X["count_state"] = (b * 3 + s).astype("float64")   # 12개 카운트 상태
    ...
    X["must_strike"] = ((b == 3) & (s < 2)).astype("float64")   # 스트라이크 넣어야 함
    X["can_waste"] = ((s == 2) & (b < 2)).astype("float64")     # 유인구 여유
    return X


def add_asof(X):
    def shrink(rate, n, prior, k):
        r = rate.fillna(prior).to_numpy()
        nn = n.to_numpy()
        return (nn * r + k * prior) / (nn + k)

    X["sh_pitcher_success"] = shrink(X.asof_pitcher_success_rate, X.asof_pitcher_n, PRIOR["success"], 200)
    X["sh_pitcher_middle"] = shrink(X.asof_pitcher_middle_rate, X.asof_pitcher_n, PRIOR["middle"], 200)
    X["sh_pitcher_ball"] = shrink(X.asof_pitcher_ball_rate, X.asof_pitcher_n, PRIOR["ball"], 200)
    X["sh_pitcher_reverse"] = shrink(X.asof_pitcher_reverse_rate, X.asof_pitcher_n, PRIOR["reverse"], 200)
    X["sh_batter_success"] = shrink(X.asof_batter_success_rate, X.asof_batter_n, PRIOR["success"], 200)
    ...
    # 매치업: 투수 제구력 - 타자가 겪은 제구력
    X["matchup_success"] = X.sh_pitcher_success - X.sh_batter_success
    # 최근 폼 (통산 대비 편차)
    for w in (1, 3, 5):
        X[f"form{w}_success"] = X[f"asof_pitcher_prev{w}_game_success_rate"] - X.sh_pitcher_success
        X[f"form{w}_middle"] = X[f"asof_pitcher_prev{w}_game_middle_rate"] - X.sh_pitcher_middle
...
def run(tag, steps):
    ...
    ref = np.mean([m.predict_proba(Xtr[last_mask])[:, 1] for m in models], axis=0).mean()
    off = logit(target_rate) - logit(ref)
    adj = 1 / (1 + np.exp(-(logit(pv) + off)))
    print(f"{tag:28s} feat={Xtr.shape[1]:3d}  raw={comp_score(pv,yva):7.1f}  "
...
run("+ 카운트", [add_count])
run("+ asof", [add_asof])
run("+ 상황", [add_situation])
run("+ 전부", [add_count, add_asof, add_situation])
